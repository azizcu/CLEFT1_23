...
from scorer.subtask_1b_en import evaluate
...
def run_majority_baseline(data_fpath, test_fpath, results_fpath):
    ...
    pipeline.fit(train_df['Text'], train_df['class_label'])

    with open(results_fpath, "w") as results_file:
        predicted_distance = pipeline.predict(test_df['Text'])
        ...
        for i, line in test_df.iterrows():
            ...
            results_file.write("{}\t{}\t{}\n".format(line['Sentence_id'], label, "majority"))
...
def run_random_baseline(data_fpath, results_fpath):
    ...
    with open(results_fpath, "w") as results_file:
        ...
        for i, line in gold_df.iterrows():
            results_file.write('{}\t{}\t{}\n'.format(line['Sentence_id'],random.choice(label_list), "random"))
...
def run_ngram_baseline(train_fpath, test_fpath, results_fpath):
    ...
    with open(results_fpath, "w") as results_file:
        ...
        for i, line in test_df.iterrows():
            label = predicted_distance[i]
            results_file.write("{}\t{}\t{}\n".format(line['Sentence_id'], label, "ngram"))
